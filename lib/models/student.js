...
var StudentSchema = new Schema({
  ...
  , deliveryCounter:        { type: Number, default: 0 }
...
StudentSchema.methods.setLastQuestion = function(qid, callback) {
  ...
  self.deliveryCounter++;

  log.debug('setting last question = ' + qid + ' for user ' + self._id + ' (counter=' + self.deliveryCounter + ')');
...
StudentSchema.methods.getNextQuestion = function(flagAsSent, options, cb) {
  ...
  var resetCounter = function(cb3) {
    if (flagAsSent) {
      self.resetCounter(lastCorrectPercentage, cb3);
    } else {
      cb2(null);
    }
  }
  ...
  // Progress reports
  if (self.deliveryCounter > 4) sendReport = true;
